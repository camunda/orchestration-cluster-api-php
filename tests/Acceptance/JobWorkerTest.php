...
final class JobWorkerTest extends TestCase
{
    ...
    public function testJobHandlerObjectsAreDispatchedAndAutoCompleted(): void
    {
        $worker = $this->worker([
            $this->activationResponse(),
            new Response(204),
        ...
        $processed = $worker->pollOnce(new class () implements JobHandler {
            public function handle(ActivatedJobResult $job, JobActionClient $action): array
            {
                return ['handledBy' => 'object-handler'];
            }
        });
        ...
        self::assertCount(2, $this->requests);
        self::assertStringEndsWith('/jobs/2251799813686200/completion', $this->requests[1]->getUri()->getPath());
        self::assertSame(['variables' => ['handledBy' => 'object-handler']], $this->jsonBody($this->requests[1]));
    }
